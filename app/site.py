...
<head>
  ...
  <body>
    ...
    <div class=wrap>
      {body_html}
      <div class=muted style="margin-top:1rem">Generated {datetime.utcnow().isoformat()}Z</div>
    </div>
  </body>
  </html>
    """
...
def devices_page(last_days: int = Query(default=7, ge=1, le=30), feature_set: str = Query(default="extended", pattern="^(basic|extended)$")):
    ...
    async function load(){{
      const r = await fetch(`/devices?last_days=__LAST_DAYS__&feature_set=__FEATURE_SET__`);
      const j = await r.json();
      ...
      const rows = list.map(d=>`<tr><td>${{d.name}}</td><td>${{(d.clusters||[d.cluster]).join(',')}}</td><td>${{d.events}}</td><td>${{fmt(d.avg_dP_kW)}}</td><td>${{fmt(d.total_energy_kWh,3)}}</td><td>${{fmt(d.energy_share_pct,1)}}</td></tr>`).join('');
      tbl.innerHTML = head + rows;
    }}
    load();
    </script>
